      ...
      <div v-else>
        <div v-for="ext in extensions" :key="ext.id" class="cyber-extension-item">
          <div class="cyber-extension-header">
            ...
            <div class="cyber-extension-actions">
              <button class="cyber-button primary" @click="executeQuery(ext)">
                <span class="button-content"> <i class="fas fa-search"></i> 执行查询 </span>
              </button>
          ...
          <div class="cyber-extension-body">
            ...
            <p>
              <strong>接口端点:</strong> <code>{{ ext.endpoint || '/query/' + ext.id }}</code>
            </p>
            ...
            <!-- 查询结果 -->
            <div
              class="cyber-result-container"
              v-if="resultVisible[ext.id]"
              :id="`result-${ext.id}`"
            >
              <div class="cyber-result-header">
                <h4>查询结果</h4>
                <button class="cyber-button small" @click="toggleResult(ext.id)">
                  <span class="button-content"> <i class="fas fa-eye-slash"></i> 隐藏结果 </span>
                </button>
              </div>
              ...
              <div v-else :id="`result-content-${ext.id}`" class="cyber-result-content">
                ...
                <div v-else-if="resultType[ext.id] === 'table'">
                  ...
                  <div
                    class="cyber-table-container"
                    v-if="resultData[ext.id] && resultData[ext.id].length > 0"
                  >
...
export default {
  ...
  methods: {
    ...
    async toggleForm(extId) {
      ...
      // 如果显示表单且还没有加载过表单内容
      if (
        this.formVisible[extId] &&
        !this.formHtml[extId] &&
        this.extensions.find((ext) => ext.id === extId).has_query_form
      ) {
    ...
    async executeQuery(ext) {
      ...
      try {
        ...
        if (formElement) {
          ...
          // 加载表单
          await this.toggleForm(extId)
          // 再请求
          await this.executeQuery(ext)
          return
        }
